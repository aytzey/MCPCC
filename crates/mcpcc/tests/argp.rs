...
#[test]
fn generates_structured_tool_from_argp() {
    let td = TempDir::new("mcpcc-argp");
...
"#;
    ...
    assert_eq!(
        props
            ...
            .and_then(|v| v.get("description"))
            .and_then(|v| v.as_str()),
        Some("Colorize output (optional arg)")
    );
    assert_eq!(
        props
            .get("args")
    ...
    // GNU argp/getopt treat a separate token as a positional instead.
    assert_eq!(
